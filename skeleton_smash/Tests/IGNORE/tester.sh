...
g++ -o smash -std=c++11 -DNDEBUG -Wall ../*.cpp
...
do
	...
	testName="${testName%.*}"
    ./smash < $i > ${i%.*}.result
	diff ${i%.*}.result ${i%.*}_expected.out > ${i%.*}_diff.txt
	#rm ${i%.*}.result
	...
	then
		...
		cp "${i%.*}_expected.out" "$FAILED_TESTS_FOLDER/$testName.out"
	fi
	valgrind --log-file=${i%.*}.valgrind_log --leak-check=full ./smash < $i > ${i%.*}.vresult 1>/dev/null 2>/dev/null
	rm ${i%.*}.vresult
	if [ -f ${i%.*}.valgrind_log ]
	then
